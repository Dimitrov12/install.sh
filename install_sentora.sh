...
# Get latest sentora
while true; do
    wget -nv -O sentora_core.zip https://github.com/sentora/sentora-core/archive/master.zip
...
if [ ! -L "/etc/zpanel" ] && [ -d "/etc/zpanel" ]; then
    ...
    rm -rf "$PANEL_PATH/sentora-core-master/cnf"
 
fi

## cp can be aliased to stop overwriting of files in centos use full path to cp
/bin/cp -rf "$PANEL_PATH/sentora-core-master/." "$PANEL_PATH/panel/"
rm sentora_core.zip
